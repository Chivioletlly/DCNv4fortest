...
正式审计要求：训练源 1183 个清晰场景、测试源 200 个清晰场景；图像必须是 1080×720 或 720×1080 的 RGB PNG。同一场景的清晰图与 11 个退化版本必须具有完全一致的尺寸、方向、颜色模式和格式。官方训练源内部允许文件名不同但清晰图内容完全相同的场景，但必须按清晰图 SHA256 组成不可拆分的内容组；官方测试源内部或官方训练源与测试源之间出现相同清晰图时仍终止审计。训练源按文件场景划分为 1083 个训练场景和 100 个验证场景：内容组按组内最低的 `SHA256("cdd11-v1:split:3407:<filename>")` 排序，只有整组能放入剩余验证名额时才纳入，直至得到恰好 100 个验证文件场景。官方测试集不参与模型选择。
